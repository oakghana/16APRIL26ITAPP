...
const TABLE = "account_unlock_requests"
const PREFIX = "AU"
...
function appendTimeline(existing: any, entry: Record<string, any>) {
  const t = Array.isArray(existing) ? existing : []
  return [...t, entry]
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { staffName, requestedById, requestedByEmail, departmentName, requesterLocation, requestDate,
      lockedSystem, otherSystemName, accountIdentifier, lockDescription, urgency,
      submittedByRole, submittedByEmail } = b

    if (!staffName || !lockedSystem || !accountIdentifier || !lockDescription) {
    ...
    const { data, error } = await supabaseAdmin.from(TABLE).insert([{
      ...
      locked_system: lockedSystem,
      other_system_name: lockedSystem === "Other" ? String(otherSystemName || "").trim() : null,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (action === "manager_assign") {
      ...
      if (!canWork(role, dept, location) && !(isUuid(actorId) && record.assigned_to_id === actorId)) {
        return NextResponse.json({ error: "Unauthorized" }, { status: 403 })
      }
      ...
      Object.assign(updateData, {
        ...
        work_notes: notes,
        submitted_for_confirmation_at: now,
      })
    } else if (action === "assignee_complete") {
      if (!canWork(role, dept) && !(isUuid(actorId) && record.assigned_to_id === actorId)) {
        return NextResponse.json({ error: "Unauthorized" }, { status: 403 })
      }
      if (!String(notes || "").trim()) return NextResponse.json({ error: "Work notes required" }, { status: 400 })
      Object.assign(updateData, {
      ...
      const isRequester = (isUuid(actorId) && record.requested_by_id === actorId)
        || String(record.staff_name || "").toLowerCase() === String(actorName || "").toLowerCase()
      if (!isRequester && (role || "").toLowerCase() !== "admin") {
        return NextResponse.json({ error: "Only requester can confirm" }, { status: 403 })
      }
      const approved = body.confirmation === "approved"
      Object.assign(updateData, {
        status: approved ? "completed" : "reopened",
        user_confirmed: approved,
        user_confirmed_by: actorName || actorProfile?.full_name || "User",
        user_confirmed_by_id: isUuid(actorId) ? actorId : null,
        user_confirmed_at: now,
        confirmation_status: body.confirmation || "approved",
        confirmation_notes: notes || null,
        closed_at: approved ? now : null,
      })
    } else {
