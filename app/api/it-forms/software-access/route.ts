...
const TABLE = "software_access_requests"
const PREFIX = "SA"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { staffName, requestedById, requestedByEmail, departmentName, requesterLocation, requestDate,
      softwareName, otherSoftwareName, accessLevel, justification, urgency,
      submittedByRole, submittedByEmail } = b

    if (!staffName || !softwareName || !justification) {
    ...
    const { data, error } = await supabaseAdmin.from(TABLE).insert([{
      ...
      other_software_name: softwareName === "Other" ? String(otherSoftwareName || "").trim() : null,
